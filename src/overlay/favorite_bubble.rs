...
use windows::core::w;
use std::sync::{Once, atomic::{AtomicBool, AtomicIsize, Ordering}};
use std::cell::RefCell;
use wry::{WebViewBuilder, Rect, WebView};
use crate::APP;
...
fn create_bubble_window() {
    unsafe {
        ...
        let hwnd = CreateWindowExW(
            ...
            w!("FavBubble"),
            WS_POPUP,
            initial_x, initial_y, BUBBLE_SIZE, BUBBLE_SIZE,
...
fn update_bubble_visual(hwnd: HWND) {
    unsafe {
        ...
        let hbm = CreateDIBSection(Some(hdc_mem), &bmi, DIB_RGB_COLORS, &mut bits, None, 0).unwrap();
        let old_bm = SelectObject(hdc_mem, hbm.into());
        
        if !bits.is_null() {
            // Draw directly to pixel buffer with anti-aliasing
            let pixels = std::slice::from_raw_parts_mut(bits as *mut u32, (BUBBLE_SIZE * BUBBLE_SIZE) as usize);
...
fn draw_bubble_pixels(pixels: &mut [u32], size: i32, is_active: bool) {
    ...
    for y in 0..size {
        for x in 0..size {
            ...
            // Anti-aliased circle
            let edge_dist = dist - radius;
            
            if edge_dist > 1.5 {
                // Outside circle - transparent
                pixels[idx] = 0;
            } else if edge_dist > -1.5 {
                // Edge region - anti-alias
                let alpha = (1.0 - (edge_dist + 1.5) / 3.0).clamp(0.0, 1.0);
                let (r, g, b) = get_bubble_color(dist / radius, is_active);
                let a = (alpha * 255.0) as u32;
                // Premultiplied alpha
                let r_pm = (r as u32 * a / 255) as u32;
                let g_pm = (g as u32 * a / 255) as u32;
                let b_pm = (b as u32 * a / 255) as u32;
                pixels[idx] = (a << 24) | (r_pm << 16) | (g_pm << 8) | b_pm;
            } else {
                // Inside circle - full color
                let (r, g, b) = get_bubble_color(dist / radius, is_active);
                // Full alpha
                pixels[idx] = (255 << 24) | ((r as u32) << 16) | ((g as u32) << 8) | (b as u32);
            }
        }
    }
    
    // Draw star on top
    draw_star_pixels(pixels, size, center as i32, center as i32, 12);
    
    // Draw border ring
    draw_border_ring(pixels, size, center, radius, is_active);
}
    ...
    for y in 0..size {
        for x in 0..size {
            ...
            if dist > border_inner && dist < border_outer + 1.0 {
                ...
                let glow_t = 1.0 - ((dist - border_inner) / (border_outer - border_inner + 1.0)).clamp(0.0, 1.0);
...
fn draw_star(hdc: HDC, cx: i32, cy: i32, size: i32) {
    unsafe {
        use std::f32::consts::PI;
        
        let outer_r = size as f32;
        let inner_r = size as f32 * 0.4;
        
        let mut points: [POINT; 10] = [POINT::default(); 10];
        
        for i in 0..10 {
            let angle = (i as f32 * PI / 5.0) - PI / 2.0;
            let r = if i % 2 == 0 { outer_r } else { inner_r };
            points[i] = POINT {
                x: cx + (r * angle.cos()) as i32,
                y: cy + (r * angle.sin()) as i32,
            };
        }
        
        // Gold fill
        let brush = CreateSolidBrush(COLORREF(0x00D7FF)); // Gold (BGR)
        let pen = CreatePen(PS_SOLID, 1, COLORREF(0x00A5E0)); // Darker gold outline
...
unsafe extern "system" fn bubble_wnd_proc(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    ...
    match msg {
        ...
        WM_LBUTTONUP => {
            let was_dragging = IS_DRAGGING.swap(false, Ordering::SeqCst);
            let _ = ReleaseCapture();
            
            // Only toggle if we weren't dragging much
            if !was_dragging || true { // Always toggle for now
...
fn show_panel(bubble_hwnd: HWND) {
    ...
    unsafe {
        ...
        let fav_count = APP.lock().map(|app| {
            app.config.presets.iter().filter(|p| p.is_favorite && !p.is_upcoming && !p.is_master).count()
        }).unwrap_or(0);
        ...
        let (panel_x, panel_y) = if bubble_rect.left > screen_w / 2 {
            (bubble_rect.left - PANEL_WIDTH - 8, bubble_rect.top - panel_height / 2 + BUBBLE_SIZE / 2)
        } else {
            (bubble_rect.right + 8, bubble_rect.top - panel_height / 2 + BUBBLE_SIZE / 2)
        };
        ...
        let panel_hwnd = CreateWindowExW(
            ...
            w!("FavPanel"),
            WS_POPUP | WS_VISIBLE,
            panel_x, panel_y.max(10), PANEL_WIDTH, panel_height,
            None, None, Some(instance.into()), None
        ).unwrap_or_default();
...
fn move_panel_to_bubble(bubble_x: i32, bubble_y: i32) {
    ...
    unsafe {
        ...
        let (panel_x, panel_y) = if bubble_x > screen_w / 2 {
            (bubble_x - PANEL_WIDTH - 8, bubble_y - panel_h / 2 + BUBBLE_SIZE / 2)
        } else {
            (bubble_x + BUBBLE_SIZE + 8, bubble_y - panel_h / 2 + BUBBLE_SIZE / 2)
        };
        
        let _ = SetWindowPos(panel_hwnd, None, panel_x, panel_y.max(10), 0, 0, SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE);
    }
}
...
fn create_panel_webview(panel_hwnd: HWND) {
    let mut rect = RECT::default();
    unsafe { let _ = GetClientRect(panel_hwnd, &mut rect); }
...
use windows::Win32::UI::Input::KeyboardAndMouse::{SetCapture, ReleaseCapture, TrackMouseEvent, TRACKMOUSEEVENT, TME_LEAVE};
